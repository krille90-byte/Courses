...
class TerminalUI:
    ...
    def show_2d(self):
        ...
        col, row = 0, 1
        for index, item in enumerate(self._tree.bfs_order_star()):
            spacing_factor = 2**(self._tree.height() - col - 1)
            if row == 1:
                print(" " * spacing_factor, end = "")
            print("   " * spacing_factor + str(item if item is not None else "*"), end = "    " * spacing_factor)
            if row >= 2**col:
                print("\n")
                col += 1
                row = 1
